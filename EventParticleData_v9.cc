...
G4bool EventParticleData::ProcessHits(G4Step* aStep,G4TouchableHistory*)
{
    ...
    ResolveSolid(aStep);
		...
		//fLastVolume = aStep->GetPreStepPoint()->GetPhysicalVolume()->GetName();
		
		
		fStepNumber = aStep->GetTrack()->GetCurrentStepNumber(); 
		  ...
		  newHit->SetStepNumber(fStepNumber);
		...
		for ( G4int i = 1; i < NoHits ; i ++ ) {
			if ((*fHitsCollection)[i]->GetIncidentEnergy()>0){
				if ((*fHitsCollection)[i]->GetTrackID() == (*fHitsCollection)[i-1]->GetTrackID()){					
					...
					// RESULTOU YAAASSS, SO NAO TA BEM PARA UM EVENTO PARA COLOCAR ABAIXO NO ELSE
					// 
					...
					if ((*fHitsCollection)[i]->GetVolume() != fScoringVolume && (*fHitsCollection)[i]->GetParticleID() == 11){
						...
						//std::cout << "Ekin_e = " << fEkin_e << std::endl;
						//std::cout << "Ekin_res = " << fEkin_res << std::endl;
					}
					...
					if (i == NoHits-1){
						...
						if (fParticleType == 2212){
							...
							if ((*fHitsCollection)[i]->GetVolume() != fScoringVolume){
								//fEkin = (*fHitsCollection)[i]->GetProtonIncidentEnergy();
								fEkin = fEkin_e - fEkin_res + (*fHitsCollection)[i]->GetProtonIncidentEnergy();
								fEkin_e = 0;
								fEkin_res = 0;
								//std::cout << "Ekin = " << fEkin << std::endl;
							}
							//
						}
					...
					if (fParticleType == 2212){ // or primary process?
						...
						fEdep_e = 0.;
						//fEdep_e = (*fHitsCollection)[i]->GetEdep();
